...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def _send(self, char):
        """
        Sends the character to the current focus object.
        - Text Editor: Uses original textInsert/Fallback logic.
        - MS Word: Uses ScanCode 0x2f to bypass language issues.
        """
        try:
            focus = api.getFocusObject()
            app_name = focus.appModule.appName.lower() if hasattr(focus, 'appModule') and hasattr(focus.appModule, 'appName') else ""
            
            # ประกาศเสียงตัวอักษรก่อนส่ง
            speech.speakText(char)

            # --- ส่วนที่ 1: เฉพาะ Microsoft Word ---
            if app_name == 'winword':
                try:
                    # ใช้การวางด้วย ScanCode 0x2f เพื่อให้ทำงานได้ทั้ง TH/EN ใน Word
                    api.copyToClip(char)
                    winUser.keybd_event(winUser.VK_CONTROL, 0, 0, 0)
                    winUser.keybd_event(0x56, 0x2f, 0, 0) # Key V + ScanCode 0x2f
                    ...
                    # Fallback สุดท้ายสำหรับ Word
                    keyboardHandler.KeyboardInputGesture.fromName("control+v").send()

            # --- ส่วนที่ 2: Browser (ยึดตามสคริปต์ที่คุณส่งมา) ---
            ...
            # --- ส่วนที่ 4: Text Editor และแอปอื่นๆ (ยึดตามสคริปต์ที่คุณส่งมาเป๊ะๆ) ---
            else:
                if hasattr(focus, 'edit') and hasattr(focus.edit, 'textInsert'):
                    focus.edit.textInsert(char)
                else:
                    # Fallback เดิมที่ทำงานได้ใน Text Editor
                    api.copyToClip(char)
                    keyboardHandler.KeyboardInputGesture.fromName("control+v").send()
    ...
    def _sendCharacters(self, chars: str):
        """
        Sends characters directly via braille input.
        This mimics the method from the En Dash Bash add-on.
        """
    ...
    # --- Script Mappings (ยึดตามต้นฉบับที่คุณส่งมาล่าสุด) ---
